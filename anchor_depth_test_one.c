...
/* global variables */
int Nvertice, Nface, Nanchor;
int *IsExist;
GLfloat *Vertices;
GLfloat *Anchors;
GLuint *Faces;
GLdouble Azimuth, Elevation;
...
void load_anchor_points(char **argv)
{
  ...
  for(i = 0; i < Nanchor; i++)
  {
    sprintf(filename, "Anchor/%s/%02d_%s.off", argv[1], atoi(argv[2]), names[i]);
...
void display(void)
{
  ...
  for(i = 0; i < Nanchor; i++)
  {
    ...
    printf("Anchor %d, z=%.2f, depth=%.2f, visible=%d\n", i, z, depth, visibility[i]);
  }
  free(visibility);
  glFlush();
}
...
int main(int argc, char** argv)
{
  ...
  sprintf(filename, "Anchor/%s/%02d.off", argv[1], atoi(argv[2]));
  /* load off file */
