% Generate the result matrix whose content is following.
% result = [a e 11 azimuth elevation distance focal px py theta error index];
% The program uses Original(normal) method
% a : groundtruth azimuth
% e : groundtruth elevation
% azimuth : estimated azimuth
% elevation : estimated elevation
% distance : estimated distance
function result = test_normal()
clc;
clear;
load('dar12345.mat');
...
load(sprintf('./mesh_images/02_plane_mesh_%d_%d_11',a,e));
...
% set results
numb = numb + 1;
result(numb,:) = [a e 11 azimuth elevation distance focal px py theta error index];
    end
end
save('result15_normal_recent.mat','result');
